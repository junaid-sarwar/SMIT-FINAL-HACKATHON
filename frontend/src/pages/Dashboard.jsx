...
import { FileText, Brain, Heart, Loader2, TrendingUp, Calendar } from "lucide-react"
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
         // 1️⃣ Fetch user data (from login cookie token)
        const userRes = await axios.get("http://localhost:8080/api/auth/me", {
          withCredentials: true,
        })
        setUserName(userRes.data.user.fullName)
        // Fetch files and insights in parallel
        const [filesRes, insightsRes] = await Promise.all([
          axios.get("http://localhost:8080/api/file/all", { withCredentials: true }),
          axios.get("http://localhost:8080/api/ai-insight", { withCredentials: true }),
        ])
        ...
        setStats({
          ...
          lastVitalsEntry: null, // placeholder (no vitals API yet)
        })

        // Latest file (most recent upload)
        if (files.length > 0) {
    ...
    <main className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="glass p-6 transition-smooth hover:shadow-xl">
          ...
          <Card className="glass p-6 transition-smooth hover:shadow-xl">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-secondary mt-2">{stats.totalInsights}</p>
              </div>
          ...
          <Card className="glass p-6 transition-smooth hover:shadow-xl">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-semibold text-accent mt-2">{stats.lastVitalsEntry || "No data"}</p>
              </div>
        ...
        {loading ? (
          ...
          <Card className="glass p-6 mb-8 transition-smooth hover:shadow-lg">
            <h2 className="text-lg font-semibold mb-4 text-foreground">Latest Upload</h2>
            <div className="flex items-center justify-between">
              <div>
              ...
              <Link to="/insights">
                <Button className="bg-primary hover:bg-primary/90 transition-smooth">View Analysis</Button>
              </Link>
            </div>
          </Card>
        ) : null}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <Card className="glass p-8 bg-gradient-to-r from-primary/10 to-secondary/10 border-primary/20 transition-smooth hover:shadow-lg">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-2xl font-bold text-foreground mb-2">Upload New Report</h2>
                <p className="text-muted-foreground">Add your medical reports for AI analysis</p>
              </div>
              <FileText className="w-8 h-8 text-primary/40" />
            </div>
            <Link to="/upload" className="mt-4 inline-block">
              <Button className="bg-primary hover:bg-primary/90 transition-smooth">Upload Report</Button>
            </Link>
          </Card>

          <Card className="glass p-8 bg-gradient-to-r from-accent/10 to-secondary/10 border-accent/20 transition-smooth hover:shadow-lg">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-2xl font-bold text-foreground mb-2">Track Your Vitals</h2>
            ...
            <Link to="/vitals" className="mt-4 inline-block">
              <Button className="bg-accent hover:bg-accent/90 transition-smooth">View Vitals</Button>
            </Link>
          </Card>
        </div>
        ...
        <Card className="glass p-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-foreground">AI Insights Summary</h2>
            </div>
            <Link to="/insights">
              <Button variant="outline" size="sm">
                View All Insights
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-lg bg-muted/50">
              ...
              <p className="text-sm text-muted-foreground">
                Your overall health metrics are improving
              </p>
            </div>
            ...
            <div className="p-4 rounded-lg bg-muted/50">
              ...
              <p className="text-sm text-muted-foreground">Recommended in 30 days</p>
            </div>
          </div>
        </Card>
      </div>
